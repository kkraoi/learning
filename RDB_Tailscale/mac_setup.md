...
> Homebrewを使う場合は `brew install --cask tailscale` でも同じものが入る。
...
## STEP 3. Microsoft Remote Desktopをインストール

1. Mac App Storeで「Microsoft Remote Desktop」を検索
2. インストール（Microsoft公式・無料）
3. アプリを起動

## STEP 4. 接続先(PC)を登録する

1. Microsoft Remote Desktopを開き、左上「+」→「PC を追加」
2. 入力項目:
...
## STEP 6. 外出先を想定した接続テスト

1. MacのWi-Fiを切り、iPhoneのテザリング等 **自宅ネットワークと異なる回線** に接続し直す
2. 同様にMicrosoft Remote Desktopから接続
...
| 画面がカクつく・重い | STEP4の「Performance」設定を回線に合わせて下げる。Retina解像度をオフにする |
