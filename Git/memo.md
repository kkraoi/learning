...
## あげてはいけないコミットをリモートリポジトリにあげた場合
1）git reset --hard 戻したいコミットID
2）git push origin 各自ブランチ --force
3）github の履歴で残ってないか確認

## あげてはいけないコミットをローカルでした場合
1）git reset HEAD^
2）ファイルを修正

## 厄介なファイル（ログファイルなど）をリモートにあげた場合
1）git revert 厄介なファイルをあげたコミットID
2）ターミナルの画面にて、リバートのメッセージを調整。
Revert "リバートメッセージ"
このコミットは、誤って追加した変更を取り消すためのものです。などの説明文
...略
3）escキー → :wq　を記入して閉じる。
